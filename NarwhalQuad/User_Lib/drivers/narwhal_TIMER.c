...
__IO int capture = 0;
__IO unsigned char ALL_PWM = 0;
OS_FlagID RADIO_PWM_INPUT_FLAG;

GPIO_TypeDef* PWM_IN_PORT[RCn] = { RC1_GPIO_PORT, RC2_GPIO_PORT, RC3_GPIO_PORT,
		RC4_GPIO_PORT };
const uint16_t PWM_IN_PIN[RCn] = { RC1_PIN, RC2_PIN, RC3_PIN, RC4_PIN };
const uint16_t PWM_IN_PINSOURCE[RCn] = { RC1_PIN_SOURCE, RC2_PIN_SOURCE,
		RC3_PIN_SOURCE, RC4_PIN_SOURCE };
const uint32_t PWM_IN_PIN_CLK[RCn] = { RC1_GPIO_CLK, RC2_GPIO_CLK,
		RC3_GPIO_CLK, RC4_GPIO_CLK };
const uint16_t PWM_IN_PIN_AF[RCn] = { RC1_GPIO_AF, RC2_GPIO_AF, RC3_GPIO_AF,
		RC4_GPIO_AF };

TIM_TypeDef * PWM_IN_TIMER[RCn] =
		{ RC1_TIMER, RC2_TIMER, RC3_TIMER, RC4_TIMER };
const uint32_t PWM_IN_TIMER_CLK[RCn] = { RC1_TIMER_CLK, RC2_TIMER_CLK,
		RC3_TIMER_CLK, RC4_TIMER_CLK };
const uint16_t PWM_IN_TIMER_IRQn[RCn] = { RC1_TIMER_IRQn, RC2_TIMER_IRQn,
		RC3_TIMER_IRQn, RC4_TIMER_IRQn };
const uint16_t PWM_IN_TIMER_PREPRIO[RCn] = { RC1_TIMER_IRQ_PREPRIO,
		RC2_TIMER_IRQ_PREPRIO, RC3_TIMER_IRQ_PREPRIO, RC4_TIMER_IRQ_PREPRIO };
const uint16_t PWM_IN_TIMER_SUBPRIO[RCn] = { RC1_TIMER_IRQ_SUBPRIO,
		RC2_TIMER_IRQ_SUBPRIO, RC3_TIMER_IRQ_SUBPRIO, RC4_TIMER_IRQ_SUBPRIO };

const uint16_t PWM_IN_TIMER_CHR[RCn] = { RC1_TIMER_CHR, RC2_TIMER_CHR,
		RC3_TIMER_CHR, RC4_TIMER_CHR };
const uint16_t PWM_IN_TIMER_CHF[RCn] = { RC1_TIMER_CHF, RC2_TIMER_CHF,
		RC3_TIMER_CHF, RC4_TIMER_CHF };
const uint16_t PWM_IN_TIMER_IRQ[RCn] = { RC1_TIMER_IRQ, RC2_TIMER_IRQ,
		RC3_TIMER_IRQ, RC4_TIMER_IRQ };
...
static inline void PWM_Timer_IRQ_Helper(PWM_CTRL_IN_TypeDef CH) {
	uint32_t TI_Rising;
	uint32_t TI_Falling;
	...
	if (TIM_GetITStatus(PWM_IN_TIMER[CH], TIM_IT_CC1) != RESET) {
		...
		/*set angle of the current controller*/
	...
	PWM_US = PWM_Timer_Duty_Helper(TI_Rising, TI_Falling);
	CTRL_RAW[CH] = PWM_US;
	//MotorControl_SetAngle(PWM_US, CH);

	//TODO: Support many inputs, currently only supports 4, here we make sure that all 4 inputs are matched before we trigger flag
	ALL_PWM |= 1 << CH;
	if (ALL_PWM == 0xF)
	{
		ALL_PWM = 0x0;
		CoEnterISR();
		CoSetFlag(RADIO_PWM_INPUT_FLAG);
		CoExitISR();
	}
#if defined(DEBUG)
...
static inline  void RC_Timer_Handler() {
	/*Check timer 1*/
	if (TIM_GetITStatus(PWM_IN_TIMER[CH1], PWM_IN_TIMER_IRQ[CH1]) != RESET) {
		PWM_Timer_IRQ_Helper(CH1);
	} else if (TIM_GetITStatus(PWM_IN_TIMER[CH2], PWM_IN_TIMER_IRQ[CH2])
			!= RESET) {
	...
	} else if (TIM_GetITStatus(PWM_IN_TIMER[CH4], PWM_IN_TIMER_IRQ[CH4])
			!= RESET) {
...
void TIM_PWM_Ctrl_In_Init(PWM_CTRL_IN_TypeDef CH) {
	...
	/*Init timer ppm input handler*/
	RADIO_PWM_INPUT_FLAG = CoCreateFlag(AUTO_RESET, NONREADY_STATE);
	if (RADIO_PWM_INPUT_FLAG < 0) {
		//TODO: Panic
	}
	...
	/* TIM4 clock enable */
	if ((PWM_IN_TIMER_CLK[CH] == RCC_APB2Periph_TIM1) || (PWM_IN_TIMER_CLK[CH]
			== RCC_APB2Periph_TIM8) || (PWM_IN_TIMER_CLK[CH]
			== RCC_APB2Periph_TIM9) || (PWM_IN_TIMER_CLK[CH]
			== RCC_APB2Periph_TIM10) || (PWM_IN_TIMER_CLK[CH]
			== RCC_APB2Periph_TIM11)) {
		RCC_APB2PeriphClockCmd(PWM_IN_TIMER_CLK[CH], ENABLE);
		uint16_t PrescalerValue = (uint16_t)((SystemCoreClock / 1) / 1000000)
				- 1;
		TIM_PrescalerConfig(PWM_IN_TIMER[CH], PrescalerValue,
				TIM_PSCReloadMode_Immediate);
	} else {
		RCC_APB1PeriphClockCmd(PWM_IN_TIMER_CLK[CH], ENABLE);
		uint16_t PrescalerValue = (uint16_t)((SystemCoreClock / 2) / 1000000)
				- 1;
		TIM_PrescalerConfig(PWM_IN_TIMER[CH], PrescalerValue,
				TIM_PSCReloadMode_Immediate);
	}
