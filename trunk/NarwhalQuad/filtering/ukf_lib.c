...
srcdkf_t *srcdkfInit(int s, int m, int v, int n, SRCDKFTimeUpdate_t *timeUpdate) {
	...
	f = (srcdkf_t *)narwhalCalloc(1, sizeof(srcdkf_t));
	...
	f->xOut = (float32_t *)narwhalCalloc(s, sizeof(float32_t));
	f->xNoise = (float32_t *)narwhalCalloc(maxN, sizeof(float32_t));
	f->xIn = (float32_t *)narwhalCalloc(s, sizeof(float32_t));
